...
function calculateContrast(backgroundColor: string): ColorToken {
  ...
  // Determine contrast color (white for dark backgrounds, dark for light backgrounds)
  const contrast = luminance > 0.5 ? '#1f2937' : '#ffffff';
  
  // Calculate contrast ratio against white and black
  const whiteLuminance = 1;
  const blackLuminance = 0;
  
  const contrastRatioWhite = (Math.max(luminance, whiteLuminance) + 0.05) / (Math.min(luminance, whiteLuminance) + 0.05);
  const contrastRatioBlack = (Math.max(luminance, blackLuminance) + 0.05) / (Math.min(luminance, blackLuminance) + 0.05);
...
function createColorScale(baseColors: Record<string, string>): ColorScale {
  ...
  Object.entries(baseColors).forEach(([key, color]) => {
    if (key === '50' || key === '100' || key === '200' || key === '300' || 
        ...
        key === '800' || key === '900') {
      result[key as keyof ColorScale] = calculateContrast(color);
    }
  });
  ...
  getWCAGCompliant: (level: 'aa' | 'aaa' = 'aa') => {
    ...
    Object.entries(colorPalette).forEach(([paletteName, palette]) => {
      ...
      Object.entries(palette).forEach(([scaleName, color]) => {
        if (color.wcag[level]) {
          compliant[paletteName][scaleName] = color;
        }
      });
    });
